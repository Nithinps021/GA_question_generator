...
7. **Answer Distribution (HARD REQUIREMENT)**: The 30 correct answers MUST be spread across all five letters A, B, C, D, and E. Target ~6 of each; the hard limits are **no letter more than 8 times and no letter fewer than 4 times**, and **every one of A, B, C, D, E must appear at least 4 times** (E is frequently neglected — make sure E is a correct answer at least 4 times). Follow this TWO-PHASE process strictly:
   - **Phase 1 — Draft correctly first**: For each question, decide the correct answer CONTENT, write the explanation for it, then place that content at ANY option slot and set the "answer" field to THAT slot's letter. At this stage, do NOT worry about letter balance — focus ONLY on making sure the "answer" letter points to the option that matches the explanation.
   - **Phase 2 — Rebalance at the end**: After all 30 questions are drafted, tally how many times each letter (A–E) appears. If any letter exceeds 8 or falls below 4, pick questions to rebalance: SHUFFLE the options array (move the correct content to a different slot), then UPDATE the "answer" field to the NEW slot letter. NEVER change just the "answer" field without also moving the option content — the answer letter and the correct option text must always stay in sync.
   - **CRITICAL**: The "answer" field must ALWAYS match the option position where the correct content sits. If the explanation says "alleviates is the antonym" and "C) Alleviates" is in slot C, then "answer" MUST be "C". Getting this wrong makes the entire question worthless.
...
2. Answer-letter count: tally the "answer" field across all 30. Each of A, B, C, D, E appears between 4 and 8 times (inclusive). If not, rebalance by SHUFFLING options (moving content between slots and updating the answer letter), never by changing only the "answer" field.
3. **ANSWER-OPTION CONSISTENCY (most critical check)**: For EVERY question, read the explanation, identify which option TEXT it describes as correct, find that text in the "options" array, and confirm the "answer" letter matches that position. Example: if the explanation says "'alleviates' is the antonym" and options are ["A) Aggravates", "B) Intensifies", "C) Alleviates", "D) Worsens", "E) Magnifies"], the answer MUST be "C". If there is ANY mismatch, fix the "answer" field to point to the correct option. Do this for ALL 30 questions — do not skip any.
